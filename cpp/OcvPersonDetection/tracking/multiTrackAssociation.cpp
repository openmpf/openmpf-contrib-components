...
void WaitingList::feed(Rect gt_win,double response)
{
	...
	for (list<Waiting>::iterator it=w_list.begin();it!=w_list.end();it++)
	{
		...
		double dis=sqrt(pow(x1-x2,2.0)+pow(y1-y2,2.0))*25;
...
void Controller::deleteObsoleteTracker(list<EnsembleTracker*>& _tracker_list, vector<MPFVideoTrack>& tracks, int frame_index)
{
	...
	*/
	...
	{	
		if((*it)->getHitFreq()*11<=MAX(l-2*sqrt(l),0))
		{
			// Kill old track here.
			tracks[(*it)->getID()].stop_frame = frame_index-1;
...
void Controller::calcSuspiciousArea(list<EnsembleTracker*>& _tracker_list)
{
	...
	{	
		if ((*it)->getAddNew() && // has new detection
			(*it)->getHitFreq()*11<l-sqrt(l) && // low hitting rate 
	...
	vector<Rect> sus_rects=waitList_suspicious.outputQualified(0.4*11);
...
/************************************************************************/
TrakerManager::TrakerManager(HogDetector* detector,Mat& frame,double thresh_promotion)
	:_detector(detector),
	_my_char(0),
	_frame_count(0),
	_tracker_count(0),
...
void TrakerManager::doHungarianAlg(const vector<Rect>& detections)
{
	...
	if (dt_size*hp_size>0)
	{
		...
		{
			Rect detect_win_GTsize=scaleWin(detections[i],0.64);
			...
			for (int j=0; j<hp_size+dt_size;j++)
			{
				if (j<hp_size)
				{
					...
					if (d<(*j_tl)->getAssRadius() /*&& ratio<1.2 && ratio>0.8*/)
					{
						...
						if (dis_to_last/(((double)(*j_tl)->getSuspensionCount()+1)/(25*5/7)+0.5)<((*j_tl)->getBodysizeResult().width*1.0))
						{								
							matrix(i,j)=d;//*h;
						}
						else
	...
	if (dt_size*hp_size>0)
	{
		...
		for (int i=0;i<dt_size;i++)
		{
			Rect detect_win_GTsize=scaleWin(detection_left[i],0.64);
			...
			for (int j=0; j<hp_size+dt_size;j++)
			{
				if (j<hp_size)
				{
					...
					{			
						...
						// ad hoc consistence enhancing rule, making association better
						if (dis_to_last/(((double)(*j_tl)->getSuspensionCount()+1)/(25*5/7)+0.5)<((*j_tl)->getBodysizeResult().width)*2)					
...
void TrakerManager::doWork(Mat& frame, int frame_index, vector<MPFVideoTrack>& tracks)
{
	...
	Mat frame_resize;
	resize(frame,frame_resize,Size((int)(frame.cols*1.0),(int)(frame.rows*1.0)));
	...
	if (detections.size()>0)
	{
		...
		{
			detection_bodysize.push_back(scaleWin(detections[i],0.64));
		}
	...
	_controller.deleteObsoleteTracker(_tracker_list, tracks, frame_index);
	...
	{	
		...
		{
			// Kill old track here.
			tracks[(*i)->getID()].stop_frame = frame_index-1;
