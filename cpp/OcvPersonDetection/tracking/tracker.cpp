	...
	//tracking_count(1)
{
	...
	_recentHitRecord=Mat::zeros(2,4*25,CV_64FC1);
}
...
void EnsembleTracker::addAppTemplate(const Mat* frame_set,Rect iniWin)
{
		...
		getIsNovice()) // if novice
	{
		...
		_result_bodysize_temp=scaleWin(iniWin,1/0.5);
		if(_retained_template != NULL) {
			delete _retained_template;
		}
...
void EnsembleTracker::calcConfidenceMap(const Mat* frame_set,Mat& occ_map)//**********************
{
	...
	double w=_window_size.width/0.5;
	double h=_window_size.height/0.5; 
	h+=2*w;
	w+=2*w;
...
void EnsembleTracker::track(const Mat* frame_set,Mat& occ_map)
{
	...
	_result_bodysize_temp=scaleWin(_result_temp,1/0.5);
...
void EnsembleTracker::calcScore()
{
	...
	Rect roi_bodysize=scaleWin(roi_result,1/0.5);
...
void EnsembleTracker::updateMatchHist(Mat& frame)
{
	...
	Rect roi_result_bodysize=scaleWin(roi_result,1/0.5);
