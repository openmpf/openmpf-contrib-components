...
public:
    ...
    AppTemplate(const cv::Mat* frame_set,      const cv::Rect iniWin,              int ID);
	//						[frame in RGB,HSV,Lab]  [initial detection window]  	       

	// calculate back-projection map
	void calcBP(const cv::Mat* frame_set, cv::Mat& occ_map,    cv::Rect ROI); 
	//                                                            [occupancy map]  
	void calcScore(cv::Rect b_inner,cv::Rect b_outer);//all argument is relative to confidence_map roi
	
	inline cv::Mat& getConfidenceMap(){return confidence_map;}
	inline cv::Point2f getShiftVector(){return shift_vector;}
