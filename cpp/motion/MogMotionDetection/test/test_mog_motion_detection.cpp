...
TEST(VideoGeneration, TestOnKnownVideo) {
    ...
    start = parameters["MOG_MOTION_START_FRAME"].toInt();
    stop = parameters["MOG_MOTION_STOP_FRAME"].toInt();
    rate = parameters["MOG_MOTION_FRAME_RATE"].toInt();
    inTrackFile = parameters["MOG_MOTION_KNOWN_TRACKS"].toStdString();
    inVideoFile = parameters["MOG_MOTION_VIDEO_FILE"].toStdString();
    outTrackFile = parameters["MOG_MOTION_FOUND_TRACKS"].toStdString();
    outVideoFile = parameters["MOG_MOTION_VIDEO_OUTPUT_FILE"].toStdString();
    comparison_score_threshold = parameters["MOG_MOTION_COMPARISON_SCORE_VIDEO"].toFloat();
    // 	Create a person tracker object.

    cout << "\tCreating a MOG Motion Detection" << endl;
    MotionDetection_MOG2 *motion_detection = new MotionDetection_MOG2();
